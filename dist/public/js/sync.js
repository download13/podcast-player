...
var SyncSettings = function (_Component) {
  _inherits(SyncSettings, _Component);

  function SyncSettings(props) {
    _classCallCheck(this, SyncSettings);

    var _this = _possibleConstructorReturn(this, (SyncSettings.__proto__ || Object.getPrototypeOf(SyncSettings)).call(this, props));
    ...
    _this.createSyncProfile = _this.createSyncProfile.bind(_this);
    _this.syncAnotherDevice = _this.syncAnotherDevice.bind(_this);
    _this.attachToExisting = _this.attachToExisting.bind(_this);
    return _this;
  }
  ...
  _createClass(SyncSettings, [{
    ...
    value: function render(props, _ref) {
      ...
      if (!token) {
        firstBtn = __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0_preact__["h"])(
          'button',
          { onClick: this.createSyncProfile },
          'Create Sync Profile'
        );
      } else {
        firstBtn = __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0_preact__["h"])(
          'button',
          { onClick: this.syncAnotherDevice },
          'Sync Another Device'
        );
      }
      ...
      return __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0_preact__["h"])(
        ...
        firstBtn,
        __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0_preact__["h"])('input', { 'class': 'code-in', placeholder: 'Access Code' }),
        __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0_preact__["h"])(
          'button',
          { onClick: function onClick(e) {
              return _this2.attachToExisting(e.target.previousSibling.value);
            } },
          'Attach to Existing Profile'
        ),
        __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0_preact__["h"])(
          'span',
          { 'class': 'status' },
    ...
    key: 'createSyncProfile',
    value: function createSyncProfile() {
      var _this3 = this;

      fetch('/sync/create', { method: 'POST' }).then(okResponse).then(function (token) {
        if (token) {
          _this3.setState({ token: token, status: 'Created new profile' });
          _this3.save(token);
        } else {
          _this3.setState({ status: 'Unable to create profile' });
        }
      });
    }
  }, {
    key: 'syncAnotherDevice',
    value: function syncAnotherDevice() {
      var _this4 = this;
      ...
      }).then(okResponse).then(function (code) {
        if (code) {
          _this4.setState({ status: 'Enter code on other device: ' + code });
        } else {
          _this4.setState({ status: 'Unable to get access code' });
        }
      });
    }
  }, {
    key: 'attachToExisting',
    value: function attachToExisting(code) {
      var _this5 = this;

      fetch('/sync/join', {
        method: 'POST',
        body: code
      }).then(okResponse).then(function (token) {
        if (token) {
          _this5.setState({ token: token, status: 'Successfully joined profile' });
          _this5.save(token);
        } else {
          _this5.setState({ status: 'Unable to join profile' });
        }
      });
    }
  }]);
...
/* harmony default export */ __webpack_exports__["a"] = SyncSettings;
